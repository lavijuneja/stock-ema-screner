...
if add_btn and stock_input:
    if stock_input.upper() not in st.session_state.stocks:
...
if st.session_state.stocks:
    st.sidebar.markdown("---")
    st.sidebar.subheader("Added Stocks")
...
if st.session_state.stocks:
    ...
    if st.button("🔍 Analyze Stocks", use_container_width=True, type="primary"):
        ...
        for idx, stock in enumerate(st.session_state.stocks):
            ...
            try:
                # Download data - get last 200 days for EMA calculation
    ...
    if st.session_state.results:
        ...
        if passing_stocks:
            st.success(f"✅ **{len(passing_stocks)} stocks qualify!**")
        else:
    ...
    st.info("👈 Stocks add karein sidebar se analysis shuru karne ke liye")
    st.markdown("""
    ### कैसे इस्तेमाल करें:
    ...
    - Stronger uptrend = EMA20 > EMA50 > EMA100 > EMA150 > EMA200
    """)
